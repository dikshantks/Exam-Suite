...
function EditTest() {
  ...
  // const category = testdetails.topic;
  ...
  //   try {
  //     console.log("hmm");
  //     const res = await axios.post("/api/test/gettests", {}, options);
  //     const testTopics = res.data.map((test) => test.topic);
  ...
  const getQuestions = () => {
    ...
    instance
      ...
      .then((res) => {
        setQuestions(
          res.data.map((question) => ({ ...question, isModified: false }))
        );
        console.log(questions);
      })
      .catch((err) => {
        console.log("error here buddy", err.response.data);
        setData(true);
      });
  };
    ...
    setQuestions((prevQuestions) =>
      prevQuestions.map((prevQuestion) =>
        prevQuestion._id === questionId
          ? {
              ...prevQuestion,
              incorrect_answers: prevQuestion.incorrect_answers.map(
                (answer, i) => (i === index ? value : answer)
              ),
              isModified: true,
            }
          : prevQuestion
  ...
  const handleAddQuestion = () => {
    ...
    instance
      ...
      .catch((err) => {
        alert.show("Question not added! Something went wrong", {
          type: "error",
        });
      });
  };
    ...
    return (
      <div>
        ...
        <Container className="mt-5">
          ...
          <Button
            className="add-button align-right buttonss"
            variant="custom"
              ...
              <Form>
                ...
                <Form.Group controlId="new-correct-answers">
                  ...
                  {newQuestion.correct_answer &&
                    newQuestion.correct_answer.map((answer, index) => (
                      <Form.Control
                        key={index}
                        type="text"
                        value={answer}
                ...
                <Form.Group controlId="new-category">
                  ...
                  <Form.Control
                    type="number"
                    value={newQuestion.category || category}
                    onChange={(event) =>
                      setNewQuestion({
                        ...newQuestion,
                ...
                <Card.Body>
                  <div className="scrollable-navi ">
                    <Card.Title className="d-flex justify-content-center">
                      <strong>Navigate Questions</strong>
                    </Card.Title>
                    <hr className="my-4" />
            ...
            {questions.map((question, index) => (
              <div className="mbb-3" key={question._id} id={question._id}>
                <Card border="secondary">
                  <Card.Body>
                    ...
                    <Form>
                      ...
                      <Form.Group
                        controlId={`incorrect-answers-${question._id}`}
                      >
                        <Form.Label>
                          <strong>Incorrect Answers</strong>
                        </Form.Label>
                ...
                {lastModifiedQuestion &&
                  lastModifiedQuestion._id === question._id && (
